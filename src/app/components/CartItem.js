    ...
    <div className="select-none">
      <div className="flex gap-x-4 mb-2">
        ...
        <div className="flex flex-1 flex-col gap-y-1">
          <div className="capitalize text-lg font-extrabold">{pizza.name}</div>
        ...
        <div className="flex flex-col justify-between">
          ...
          <div>
            <span className="text-[17px] font-medium font-robotoCondensed">GH&#162;{parseFloat(pizza.price * pizza.amount).toFixed(2)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 p-6 border-b border-black/10">
       ...
       {pizza.additionalTopping.map((topping,index)=> {
          return <div className="capitalize text-sm gradient font-medium px-3 py-1 rounded-full leading-none">
            {topping.name}
          </div>
